//
//  CategoryPickerView.swift
//  ClickmeliveHostCoreIOS
//
//  Created by Can Kaçmaz on 9.06.2022.
//

import UIKit
import IQKeyboardManagerSwift
import ClickmeliveHostCore

extension CategoryPickerView {
    func populate(with categories: [EventCategory]) {
        self.categories = categories
    }
    
    @objc private func categorySelected() {
        tfCategories.resignFirstResponder()
        
        guard !categories.isEmpty else { return }
        
        let selectedCategory = categories[pickerView.selectedRow(inComponent: 0)]
        tfCategories.text = selectedCategory.name
    }
    
    func getSelectedCategoryId() -> Int? {
        guard !categories.isEmpty else { return nil }
        let selectedCategory = categories[pickerView.selectedRow(inComponent: 0)]
        return selectedCategory.id
    }
}

final class CategoryPickerView: UIView {
    
    private var categories: [EventCategory] = []
    
    private let categoriesStackView: UIStackView = {
        let stackView = UIStackView()
        stackView.axis = .vertical
        stackView.distribution = .fill
        stackView.spacing = 8
        return stackView
    }()
    
    let lblCategories: UILabel = {
        let label = UILabel()
        label.font = UIFont(name: Fonts.semibold, size: 12)
        label.textColor = Colors.primaryText
        return label
    }()
    
    let tfCategories: UITextField = {
        let tf = CMLTextField(padding: 15,
                              font: UIFont(name: Fonts.regular, size: 14)!,
                              cornerRadius: 4)
        tf.keyboardToolbar.doneBarButton.tintColor = Colors.primary
        tf.keyboardToolbar.doneBarButton.setTitleTextAttributes([NSAttributedString.Key.foregroundColor: Colors.primary,
                                                                        NSAttributedString.Key.font: UIFont(name: Fonts.medium, size: 14)!], for: UIControl.State.normal)
        return tf
    }()
    
    let pickerView: UIPickerView = {
        let view = UIPickerView()
        return view
    }()
    
    override init(frame: CGRect) {
        super.init(frame: frame)
        
        tfCategories.addRightButtonOnKeyboardWithText("Seç", target: self, action: #selector(categorySelected))
        
        tfCategories.inputView = pickerView
        pickerView.delegate = self
        pickerView.dataSource = self
        
        setupViews()
        setupLayout()
    }
    
    required init?(coder: NSCoder) {
        fatalError("init(coder:) has not been implemented")
    }
    
    private func setupViews() {
        [lblCategories, tfCategories].forEach {
            categoriesStackView.addArrangedSubview($0)
        }
        
        addSubview(categoriesStackView)
    }
    
    private func setupLayout() {
        categoriesStackView.fillSuperview()
        tfCategories.constrainHeight(50)
    }
}

extension CategoryPickerView: UIPickerViewDelegate, UIPickerViewDataSource {
    public func numberOfComponents(in pickerView: UIPickerView) -> Int {
        return 1
    }

    public func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        return categories.count
    }

    public func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        return categories[row].name
    }
}

