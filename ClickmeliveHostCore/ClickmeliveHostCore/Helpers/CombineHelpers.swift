...
public extension HTTPClient {
    ...
    private var TOKEN_HEADER_FIELD: String { return "Token"}
    
    func getPublisher(urlRequest: URLRequest, withAuthentication loader: AuthTokenLoader? = nil) -> Publisher {
        var task: HTTPClientTask?
        return Deferred {
            Future { completion in
                guard let token = loader?.load() else {
                    task = self.execute(with: urlRequest, completion: completion)
                    return
                }
                
                var authenticatedRequest = urlRequest
                authenticatedRequest.addValue(token, forHTTPHeaderField: TOKEN_HEADER_FIELD)
                task = self.execute(with: authenticatedRequest, completion: completion)
            }
        }
